...
export default function CreateStockTrade() {
    ...
    const handleSubmit = async (event) => {
        ...
        form.append("mutual_fund_id",query.symbol);
        ...
        const response = await inputService.registerMfTrade(form).then((res) => {
            setLoading(false);
            router.push('/')
        }).catch((err) => {
            setLoading(false);
    ...
    return (
        <FormLayout name="Stock Trade ">
        <form >   
            <div className="relative">
                <input type="search"  className="block p-4  w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-500" placeholder="Search By Vendor Name, Id, Gstr"
                 onChange={handleQuery}/>
                <button type="submit" className="text-white absolute right-2.5 bottom-2.5 bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800"  
                ></button>   
            </div>
        </form>
